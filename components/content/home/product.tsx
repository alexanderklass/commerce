...
export default function Product({ product, newProduct }: newProductProps) {
  ...
  const [selectedVariant, setSelectedVariant] = useState(handleVariantStartUpValue());
  const [selectedVariantPrice, setSelectedVariantPrice] = useState(product.priceRange.minVariantPrice.amount);
  return (
    <div
      className={
    ...
    >
      ...
      <div className={'flex min-h-[180px] w-full flex-col justify-between p-4 text-black'}>
        <p>{product.title}</p>
        <div className={'flex w-full flex-col gap-y-4'}>
          ...
          <div className={'flex w-full flex-row items-center justify-between'}>
            <Price amount={selectedVariantPrice} currencyCode={product.priceRange.minVariantPrice.currencyCode} />
